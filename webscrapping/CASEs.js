...
async function fetchCASEs(collectionName, containerClass, priceClass) {
    ...
    do {
        const url = `https://www.x-kom.pl/g-5/c/389-obudowy-do-komputera.html?page=${page}&per_page=60&sort_by=rating_desc`;
...
async function runCASEs(time) {
    console.log('Starting runCASEs...')
    const interval = setInterval(async () => {
        console.log('CASEs interval has been started...')
        await fetchCASEs('CASEs', '.sc-1yu46qn-4', '.sc-6n68ef-3').then(() => console.log('CASEs fetch is completed.')).catch(err=> {
            clearInterval(interval)
            console.log(err)
        })
        console.log('CASEs interval has been finished...')
    },time)
}
