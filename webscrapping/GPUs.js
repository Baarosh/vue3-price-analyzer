...
async function fetchGPUs(collectionName, containerClass, priceClass) {
    ...
    do {
        const url = `https://www.x-kom.pl/g-5/c/345-karty-graficzne.html?page=${page}&sort_by=rating_desc`;
...
async function runGPUs(time) {
    console.log('Starting runGPUs...')
    const interval = setInterval(async () => {
        console.log('GPUs interval has been started...')
        await fetchGPUs('GPUs', '.sc-1yu46qn-4', '.sc-6n68ef-3').then(() => console.log('GPUs fetch is completed.')).catch(err=> {
            clearInterval(interval)
            console.log(err)
        })
        console.log('GPUs interval has been finished...')
    },time)
}

export default runGPUs