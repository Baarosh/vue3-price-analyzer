...
async function fetchMOBOs(collectionName, containerClass, priceClass) {
    ...
    do {
        const url = `https://www.x-kom.pl/g-5/c/14-plyty-glowne.html?page=${page}&per_page=60&sort_by=rating_desc`;
...
async function runMOBOs(time) {
    console.log('Starting runMOBOs...')
    const interval = setInterval(async () => {
        console.log('MOBOs interval has been started...')
        await fetchMOBOs('MOBOs', '.sc-1yu46qn-4', '.sc-6n68ef-3').then(() => console.log('MOBOs fetch is completed.')).catch(err=> {
            clearInterval(interval)
            console.log(err)
        })
        console.log('MOBOs interval has been finished...')
    },time)
}

export default runMOBOs