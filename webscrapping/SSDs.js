...
async function runSSDs(time) {
    console.log('Starting runSSDs...')
    const interval = setInterval(async () => {
        console.log('SSDs interval has been started...')
        await fetchSSDs('SSDs', '.sc-1yu46qn-4', '.sc-6n68ef-3').then(() => console.log('SSDs fetch is completed.')).catch(err=> {
            clearInterval(interval)
            console.log(err)
        })
        console.log('SSDs interval has been finished...')
    },time)
}

export default runSSDs